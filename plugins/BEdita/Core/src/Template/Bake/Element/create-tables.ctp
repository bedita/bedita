    ...
    foreach ($constraints[$tableArgForArray] as $name => $constraint):
        ...
        if ($constraint['columns'] !== $primaryKeysColumns): %>
            ->addIndex(
                ...
                [<%
                    $suffix = ($constraint['type'] === 'unique') ? '_uq' : '_idx';
                    $params = ['name' => preg_replace('/_fk$/', $suffix, $name)];
...
foreach($this->Migration->indexes($tableArgForMethods) as $name => $index):
    sort($foreignKeys);
    ...
    if (!in_array($indexColumns, $foreignKeys)):
        %>
            ->addIndex(
                ...
                %>],
                [<%
                    $params = ['name' => preg_replace('/_fk$/', '_idx', $name)];
