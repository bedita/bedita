...
class NewsletterController extends ModulesController {
	...
	function viewInvoice($id) {
		if (empty($id)) {
			throw new BeditaException(__("No invoice selected"), "No invoice selected");
		}
	...
	function save() {
		$this->saveMessage();
 		$this->userInfoMessage(__("Mail message saved")." - ".$this->request->data["title"]);
		$this->eventInfo("mail message [". $this->request->data["title"]."] saved");
	}

	public function delete() {
		$this->checkWriteModulePermission();
		$objectsListDeleted = $this->deleteObjects("MailMessage");
		$this->userInfoMessage(__("Mail message deleted") . " -  " . $objectsListDeleted);
	...
	public function viewMailGroup($id=null) {
		...
		if($id!=null) {
			...
			$this->request->params['toolbar'] = &$card['toolbar'] ;
	...
	public function saveMailGroups() {
		$this->checkWriteModulePermission();
		if(empty($this->request->data["MailGroup"]["group_name"]))
			throw new BeditaException( __("Missing list name"));
		if(empty($this->request->data["MailGroup"]["area_id"]))
 			throw new BeditaException( __("Missing publication"));
		$this->Transaction->begin() ;
		if(!$this->MailGroup->save($this->request->data)) {
			throw new BeditaException(__("Error saving mail group"), $this->MailGroup->validationErrors);
		}

		$mail_group_id = $this->MailGroup->id;

		// add subscribers
		if (!empty($this->request->params["form"]["addsubscribers"])) {
			$subscribers = explode(",", $this->request->params["form"]["addsubscribers"]);
			foreach ($subscribers as $sub) {
				...
				if ( !($card_id = $this->Card->field("id", array("newsletter_email" => $sub))) ) {
					...
					if (!$this->Card->save($dataCard))
						throw new BeditaException(__("Error adding subscribers"), $this->Card->validationErrors . "Email: " . $sub);
				...
				// join group, if not already joined
					...
					$dataJoin["MailGroupCard"]["hash"] = md5($card_id . microtime() . $this->request->data["MailGroup"]["group_name"]);

					$this->MailGroupCard->create();
					if (!$this->MailGroupCard->save($dataJoin))
						throw new BeditaException(__("Error on join between card and mail group"), $this->MailGroupCard->validationErrors);
				}

			}
		}
		$this->Transaction->commit();
		$this->userInfoMessage(__("Mail Group saved")." - ".$this->request->data["MailGroup"]["group_name"]);
		$this->eventInfo("mail group [" .$this->request->data["MailGroup"]["group_name"] . "] saved");
	}

	public function deleteMailGroups() {
		$this->checkWriteModulePermission();
		if(empty($this->request->data["MailGroup"]["id"]))
 	 	    throw new BeditaException( __("No data"));
 	 	$this->Transaction->begin() ;
		if(!$this->MailGroup->delete($this->request->data["MailGroup"]["id"])) {
			throw new BeditaException(__("Error saving mail group"), $this->MailGroup->validationErrors);
		}
		$this->Transaction->commit();
		$this->userInfoMessage(__("Mail Group deleted") . " -  " . $this->request->data["MailGroup"]["group_name"]);
		$this->eventInfo("mail group " . $this->request->data["MailGroup"]["id"] . "-" . $this->request->data["MailGroup"]["group_name"] . " deleted");
	}

	public function sendNewsletter() {
		if (empty($this->request->data["MailGroup"]))
			throw new BeditaException(__("Missing invoices"));
		if (empty($this->request->data["start_sending"]))
			throw new BeditaException(__("Missing sending date"));

		$this->request->data["mail_status"] = "pending";
		$this->saveMessage();
		$this->userInfoMessage(__("Mail ready to be sent on ") . $this->request->data["start_sending"]);
		$this->eventInfo("mail [". $this->request->data["title"]."] prepared for sending");
	}

	public function testNewsletter($to) {
		$this->saveMessage();
		// send html test
		$this->BeMail->sendMailById($this->request->data["id"],$to, true);
		// send txt test
		$this->BeMail->sendMailById($this->request->data["id"],$to, false);
		$this->userInfoMessage(__("Test mail sent to ") . $to);
		$this->eventInfo("test mail [". $this->request->data["title"]."] sent");
	}

	private function saveMessage() {
		$this->checkWriteModulePermission();
		if (empty($this->request->data["MailGroup"]))
			$this->request->data["MailGroup"] = array();
		if (empty($this->request->data["subject"])) {
			$this->request->data["subject"] = $this->request->data["title"];
		}
	...
	public function saveTemplate() {
		...
		$this->saveObject($this->MailTemplate);
	 	$this->Transaction->commit() ;
 		$this->userInfoMessage(__("Mail template saved")." - ".$this->request->data["title"]);
		$this->eventInfo("mail template [". $this->request->data["title"]."] saved");
	}

	public function cloneObject() {
		$object_type_id = ClassRegistry::init("BEObject")->findObjectTypeId($this->request->data['id']);
		unset($this->request->data['id']);
		$this->request->data['status']='draft';
		$this->request->data['fixed'] = 0;
		if ($object_type_id == Configure::read("objectTypes.mail_message.id")) {
			if (!empty($this->request->data['start_sending']) && !empty($this->request->data['MailGroup'])) {
				$this->request->data['mail_status'] = 'pending';
			} else {
				$this->request->data['start_sending'] = "";
				$this->request->data['start_sending_time'] = "";
				$this->request->data['MailGroup'] = array();
			}
			$this->save();
		} elseif ($object_type_id == Configure::read("objectTypes.mail_template.id")) {
			$this->saveTemplate();
			$this->request->action = "cloneTemplate";
		}
	}
	...
	function showTemplateDetailsAjax($id) {
		...
		$this->render(null, null, APP . 'View' . DS . "newsletter/inc/form_message_details.tpl");
	}

	function loadContentToNewsletter($template_id=null) {
		$objects = array();
		$contents_id = explode( ",", trim($this->request->params["form"]["object_selected"],","));
		...
		foreach ($contents_id as $id) {
			...
			if (!empty($this->request->params["form"]["txt"])) {
		...
		if (!empty($template_id)) {
			$field = (!empty($this->request->params["form"]["txt"]))? "abstract" : "body";
		...
		$tpl = (empty($this->request->params["form"]["txt"]))? "contents_to_newsletter_ajax.tpl" : "contents_to_newsletter_txt_ajax.tpl";
		$this->render(null, null, APP . 'View' . DS . "newsletter/inc/" . $tpl);
	}
	...
	public function listSubscribers($mail_group_id) {
		...
		$this->request->params['toolbar'] = &$card['toolbar'] ;
		$this->layout = null;
		$this->render(null, null, APP . 'View' . DS . "newsletter/inc/list_subscribers.tpl");
	}
	...
	public function addCardToGroup($old_group_id=null) {
		...
		if(!empty($this->request->params['form']['objects_selected'])) {
			...
			$groupname = $this->MailGroup->field("group_name", array("id" => $this->request->params["form"]["destination"]));

			$data["MailGroupCard"]["mail_group_id"] = $this->request->params["form"]["destination"];
			$data["MailGroupCard"]["status"] = "confirmed";

			foreach ($this->request->params['form']['objects_selected'] as $card_id) {

				// move to group => delete from previous group
				if ($this->request->params["form"]["operation"] == "move" && !empty($old_group_id)) {
				...
				$join_id = $this->MailGroupCard->field("id", "card_id=".$card_id." AND mail_group_id=".$this->request->params["form"]["destination"] );
				...
				if (!$join_id) {
					...
					if (!$this->MailGroupCard->save($data))
						throw new BeditaException(__("Error adding subscriber " . $card_id . " in group " . $data["MailGroupCard"]["mail_group_id"]));
				}

			}

			$this->Transaction->commit();
			//$this->userInfoMessage(__("Subscribers associated to list") . " - " . $groupname);
			$this->eventInfo("Subscribers associated to list " . $this->request->params["form"]["destination"]);
		}
	...
	public function changeCardStatus($mail_group_id=null) {
		...
		if(!empty($this->request->params['form']['objects_selected'])) {
			...
			$this->Transaction->begin() ;
			foreach ($this->request->params['form']['objects_selected'] as $id) {
				$this->checkObjectWritePermission($id);
				$this->Card->id = $id;
				if(!$this->Card->saveField('mail_status',$this->request->params['form']["newStatus"]))
					throw new BeditaException(__("Error saving status for item: ") . $id);
			}
			$this->Transaction->commit() ;

			$this->eventInfo("Change mail_status to " . $this->request->params['form']["newStatus"] . " at subscribers  " . implode(",",$this->request->params["form"]["objects_selected"]));
		}
	...
	public function unlinkCard($mail_group_id) {
		...
		if(!empty($this->request->params['form']['objects_selected'])) {
			$this->MailGroupCard->deleteAll(array(
					...
					"card_id" => $this->request->params['form']['objects_selected']
				)
			);
		}
		$this->MailGroup->id = $mail_group_id;
