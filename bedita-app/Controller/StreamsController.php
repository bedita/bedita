...
class StreamsController extends AppController {
	...
	public function get_item_form($filename = null) {
		$filename = urldecode($this->request->params['form']['filename']) ;
		if(!($id = $this->Stream->getIdFromFilename($filename))) throw new BeditaException(sprintf(__("Error getting id object: %d"), $id));
	...
	public function get_item_form_by_id($id =null) {
		$this->_get_item_form($this->request->params['form']['id']) ;
		if (!empty($this->request->params['form']['template'])) {
			$this->render($this->request->params['form']['template']);
		}
	}
	
	private function _get_item_form($id) {
		$conf  = Configure::getInstance() ;
		foreach ($this->request->params['form'] as $k =>$v) {
			$$k = $v ;
		}
		$rec = $this->BEObject->recursive ;
		$this->BEObject->recursive = -1 ;
		if(!($ret = $this->BEObject->read('object_type_id', $id))) 
			throw new BeditaException(sprintf(__("Error getting object: %d"), $id));
