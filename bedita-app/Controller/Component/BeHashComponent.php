 ...
 */
class BeHashComponent extends Component {
	...
	public function handleHash($service_type, $hash=null) {
		...
		if (!empty($hash)) {
			...
			if (!empty($hashRow["command"])) {
				if (empty($this->controller->params["named"]["command"]) || $hashRow["command"] != $this->controller->params["named"]["command"]) {
					throw new BeditaException(__("Incorrect hash command"));
				}
		...
		// first hash operation
			...
			if (empty($service_type)) {
				throw new BeditaException(__("missing service type"));
			}
		...
		if (!method_exists($this, $method)) {
			throw new BeditaException(__("missing method to manage hash case"));
		}
		...
		if ($this->closeHashJob && !empty($hashRow["status"]) && $hashRow["status"] == "pending") {
			...
			if(!$hashModel->saveField("status", "closed")) {
				throw new BeditaException(__("Error saving hash status"));
			}
		}
		if (!empty($mailParams)) {
			try {
				...
				throw new BeditaHashException(__("Error sending notification mail"));
			}
		}
		return true;
	...
	public function getHashRow($hash) {
		...
		if ( empty($hashRow["HashJob"]) || empty($hashRow["HashJob"]["service_type"]) || $hashRow["HashJob"]["status"] == "expired") {
			$this->controller->Session->setFlash(__("Hash not valid or expired."), "message", array("class" => "info"), 'info');
			return false;
		}
	...
	protected function userSignUp($data) {
		if (empty($data['User'])) {
			throw new BeditaHashException(__("Error on sign up: no data"));
		}

		if (empty($data['User']['email'])) {
			throw new BeditaHashException(__("Error on sign up: an email is required"));
		}

		if (!$this->controller->BeAuth->checkConfirmPassword($this->controller->params['form']['pwd'], $data['User']['passwd'])) {
			throw new BeditaHashException(__("Passwords mismatch"));
		}
		...
		if (!empty($data["Card"])) {
			...
			if (!ClassRegistry::init("Card")->save($data["Card"])) {
				throw new BeditaHashException(__("Error saving data"));
			}
		}
		...
		if ($userModerateSignup === true) { // true but not email, the user is created not valid and a mail is sent to the user
			...
			if (!$hash_job->save($data["HashJob"])) {
				throw new BeditaException(__("Error generating hash confirmation for " . $user["User"]["userid"]));
			}
	...
	protected function userSignUpActivation($data) {
		...
		if (empty($user)) {
			throw new BeditaHashException(__("Hash isn't valid or user doesn't exist."));
		}

		$user["User"]["valid"] = 1;

		if (!$userModel->save($user)) {
			throw new BeditaException(__("Error saving user."));
		}
	...
	protected function newsletterSubscribe($data) {
		if(empty($data)) {
			throw new BeditaHashException(__("Error on subscribing: no data"));
		}
		if(empty($data['newsletter_email'])) {
			throw new BeditaHashException(__("Error on subscribing: empty 'newsletter_email'"));
		}
		if(empty($data['joinGroup'])) {
			throw new BeditaHashException(__("Error on subscribing: no mail_group indicated"));
		}
		...
		if(empty($c)) {
			...
			$card->create();
			if(!($card->save($data))) {
				throw new BeditaException(__("Error saving card for " . $newsletter_email));
			}
		...
		foreach($newsletters as $n) {
			...
			if(!empty($m)) {
				...
				if (empty($mailGroup)) {
					throw new BeditaException(__("Error loading mail group detail: id=" . $n['mail_group_id']));
				}
				...
				if ($mailGroup["security"] == "all") {
					...
					$hash_job->create();
					if (!$hash_job->save($data["HashJob"])) {
						throw new BeditaException(__("Error generating hash confirmation for " . $newsletter_email));
					}
				...
				if (!$mail_group_card->save($joindata)) {
					throw new BeditaException(__("Error subscribing " . $newsletter_email));
				}
	...
	protected function newsletterSubscribeConfirm($data) {
		if ($data["HashJob"]["status"] != "pending")
			throw new BeditaHashException(__("Hash isn't valid."));
		
		if (empty($data["HashJob"]["mail_group_id"]))
			throw new BeditaHashException(__("Missing mail group id"));
		...
		if (empty($m)) {
			throw new BeditaHashException(__("User not associated to mailgroup or he isn't in pending status"));
		}
		
		$mail_group_card->id = $m['MailGroupCard']['id'];
		if(!$mail_group_card->saveField("status", "confirmed")) {
			throw new BeditaException(__("Error saving mail group card"));
		}
		
		$beObject = ClassRegistry::init("BEObject");
		$beObject->id = $data["HashJob"]['user_id'];
		if (!$beObject->saveField("status", "on")) {
			throw new BeditaException(__("Error setting on Card status"));
		}
	...
	protected function newsletterUnsubscribe($data) {
		...
		if (empty($data['card_id']))
			throw new BeditaHashException(__("missing card id"));
		...
		if (!empty($hashjobs)) {
			foreach ($hashjobs as $hashRow) {
				if (!empty($hashRow["HashJob"]["mail_group_id"]) 
					...
					throw new BeditaHashException(__("hashjob for unsubscribe already in use"));
				}
			}
		}
		...
		if(empty($m)) {
			throw new BeditaHashException(__("User is not subscribed to mail group"));
		}		
	...
	protected function newsletterUnsubscribeConfirm($data) {
		...
		if (empty($m)) {
			throw new BeditaHashException(__("User not associated to mailgroup or he is in pending status"));
		}

		if(!$mail_group_card->delete($m['MailGroupCard']['id'])) {
			throw new BeditaException(__("Error removing association mail group card"));
		}
	...
	protected function recoverPassword($data) {
		...
		if (empty($data["email"])) {
			throw new BeditaHashException(__("Missing email to send recover instructions"));
		}
		...
		if (empty($user["Group"])) {
			throw new BeditaHashException(__("No user with access privileges found"));
		}
	...
	protected function recoverPasswordChange($data) {
		...
		if (empty($userToChangePwd) || $userToChangePwd["User"]["id"] != $data["HashJob"]["user_id"]) {
			...
			if (empty($user)) {
				throw new BeditaHashException(__("User not found"));
			}
			...
			if (empty($data["User"]["passwd"])) {
				throw new BeditaHashException(__("Missing data"));
			}
			...
			if (!$this->controller->BeAuth->changePassword($userToChangePwd["User"]["userid"], $pwd)) {
				throw new BeditaHashException(__("Error updating password"));
			}
