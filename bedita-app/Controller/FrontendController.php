...
abstract class FrontendController extends AppController {
	...
	protected function checkLogin() {
		...
		if(!$this->BeAuth->isLogged()) {
			...
			if (!empty($this->request->params["form"]["login"])) {
				$userid 	= (isset($this->request->params["form"]["login"]["userid"])) ? $this->request->params["form"]["login"]["userid"] : "" ;
				$password 	= (isset($this->request->params["form"]["login"]["passwd"])) ? $this->request->params["form"]["login"]["passwd"] : "" ;

				if(!$this->BeAuth->login($userid, $password, null, $frontendGroupsCanLogin)) {
					//$this->loginEvent('warn', $userid, "login not authorized");
					$this->userErrorMessage(__("Wrong username/password or session expired"));
					$this->logged = false;
				} else {
					$this->eventInfo("FRONTEND logged in publication");
				}
				$redirect = (!empty($this->request->params["form"]["backURL"]))? $this->request->params["form"]["backURL"] : $this->loginRedirect;
		 ...
		 * throws exception
		 */
		if ( (!$this->logged && Configure::read("staging") === true) || ((empty($this->request->params["pass"][0]) || $this->request->params["pass"][0] != "logout") && !$this->publication["authorized"])) {
	...
	protected function accessDenied($type) {
		if ($type == self::UNLOGGED && !strstr($this->request->here,"/login")) {
			$message = __("You have to be logged to access that item");
			$this->userInfoMessage($message);
		} elseif ($type == self::UNAUTHORIZED) {
			$message = __("You aren't authorized to access that item");
		...
		if(!empty($forward)) {
			if (substr($forward, 0, 5) != "http:") {
				...
				if (!empty($this->request->params["pass"][2])) {
					$forward .= "/" . implode("/", array_slice($this->request->params["pass"],2));
				}
			}
	...
	* */
	protected function loadSectionsTree($parentName, $loadContents = false, array $exclude_nicknames = array(), $depth = null, $flatMode = false) {
		...
		if (empty($parent_id)) {
			throw new BeditaException(__("Error loading sections tree. Missing parent" . ": " . $parentName));
		}
	...
	public function georss($sectionName) {
		$gml = (!empty($this->request->params['named']['gml']));
		$this->section($sectionName);
	...
	public function section($secName, $contentName = null) {
		...
		if(!empty($contentName)) {
			...
			// check that contentName is a child of secName
				...
				throw new BeditaException(__("Content " . $contentName . " doesn't belong to " . $secName));
			}
		...
		$this->sectionOptions["childrenParams"] = array_merge($this->sectionOptions["childrenParams"], $this->request->params["named"]);
...
*/
	...
	public function route() {
		...
		$this->request->action = $methodName;
		...
		try {
			...
			// launch 404 error
			throw new BeditaException(__("Content not found"), $ex->getMessage());
		}
	...
	public function search() {
		...
		$this->searchOptions = array_merge($this->searchOptions, $this->request->params["named"]);
		$s = $this->BEObject->getStartQuote();
		$e = $this->BEObject->getEndQuote();
		// add rules for start and end pubblication date
			...
			if($type != "toolbar") {
				...
				foreach ($archive[$type] as $year => $month) {
					...
					foreach ($month as $key => $i) {
						...
						$archive[$type][$year][$key]["monthName"] = __($monthName[$key]);
					}
	...
	private function loadObjectsByTagCategory($name, $options=array(), $type = "tag") {
		...
		if($type === "tag") {
			...
			if (empty($detail))
				throw new BeditaException(__("No tag found"). " - $name");

			$options = array_merge($this->tagOptions, $options, $this->request->params["named"]);
			$filter["tag"] = $name;
			...
			if (empty($detail))
				throw new BeditaException(__("No category found") . " - $name");

			$options = array_merge($this->tagOptions, $options, $this->request->params["named"]);
			$filter["category"] = $name;

		} else {
			throw new BeditaException(__("Unsupported type"). " - $type");
		}
	...
	protected function loadAnnotations($annotationType, $objectName, $options=array()) {
		...
		if (empty($annotationType) || empty($objectName))
			throw new BeditaException(__("Annotation type or object_id missing"));

		$object_id = (is_numeric($objectName))? $objectName : $this->BEObject->getIdFromNickname($objectName);

		$options = array_merge($this->annotationOptions[$annotationType], $options, $this->request->params["named"]);
	 ...
	 *
	 * If it's ajax request and if not empty $this->request->params["form"]["render"] renders it
	 *
	...
	public function saveComment() {
		...
		if (!empty($this->request->data)) {
			...
			$this->request->data = BeLib::getInstance()->stripData($this->request->data);

			if(!isset($this->Comment)) {
				$this->Comment = $this->loadModelByType("Comment");
			}
			$this->request->data["title"] = substr($this->request->data["description"],0,30) . "...";
			// for comment status check contents.comments
			$beObject = ClassRegistry::init("BEObject");
			$commentsFlag = $beObject->field("comments", array("id" => $this->request->data['object_id']));
			if($commentsFlag == 'moderated') {
				 ...
				 throw new BeditaException(__("Post comment disabled"));
			}
			...
			try {
        		...
        		if($bannedIP->isBanned($_SERVER['REMOTE_ADDR'])) {
					throw new BeditaException(__("Error saving comment"));
        		}
				...
				// set User data
					...
					$this->request->data["user_created"] = $userdata["id"];
					$this->request->data["user_modified"] = $userdata["id"];
					$this->request->data["author"] = (!empty($userdata["realname"]))? $userdata["realname"] : $userdata["userid"];
					if ( (trim($userdata["email"]) != "") ) {
						$this->request->data["email"] = $userdata["email"];
					}
				}

				// build thread path
				if (!empty($this->request->params["form"]["thread_parent_id"])) {
					$thread_path = $this->Comment->field("thread_path", array("id" => $this->request->params["form"]["thread_parent_id"]));
					$this->request->data["thread_path"] = (!empty($thread_path))? $thread_path . "/" . $this->request->params["form"]["thread_parent_id"] : "/" . $this->request->params["form"]["thread_parent_id"];
				}

				// content url shown in notification
				if (empty($this->request->data["notification_content_url"])) {
					$this->request->data["notification_content_url"] = $this->publication["public_url"] . $this->referer();
				}
				...
				$this->Transaction->commit();
				$this->userInfoMessage(__($userMsgOK));
			} catch (BeditaException $ex) {
				$this->Transaction->rollback();
		...
		if($this->RequestHandler->isAjax()) {
			...
			if (!empty($this->request->params["form"]["render"])) {
				$this->render(null, null, $this->request->params["form"]["render"]);
			}
		} else {
	...
	public function printme($id=null, $printLayout=null) {
		...
		if (!empty($this->request->params["form"]["printLayout"]))
			$id = $this->request->params["form"]["printLayout"];
	 ...
	 * 		  If undefined get object type from $this->request->data["object_type_id"]
	...
	protected function save($modelName=null) {
		...
		try {
			if (empty($modelName) && empty($this->request->data["object_type_id"])) {
				throw new BeditaException(__("no object type defined"));
			}
			$modelName = (empty($modelName))? Configure::read("objectTypes.".$this->request->data["object_type_id"].".model") : $modelName;
			...
			$this->Transaction->commit();
			$this->userInfoMessage(__($modelName . " saved"));
	...
	protected function delete() {
		...
		try {
			if (!empty($this->request->data["object_type_id"])) {
				$object_type_id = $this->request->data["object_type_id"];
			} elseif (!empty($this->request->data["id"])) {
				$object_type_id = $this->BEObject->findObjectTypeId($this->request->data["id"]);
			} else {
				throw new BeditaException(__("no object type defined"));
			}
			...
			$this->userInfoMessage(__($objectsDeleted . " deleted"));
			return true;
		} catch (BeditaException $ex) {
			$this->log($ex->errorTrace());
			$this->userErrorMessage($ex->getMessage());
