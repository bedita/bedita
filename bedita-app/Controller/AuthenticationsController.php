...
class AuthenticationsController extends AppController {
	...
	function login() {
		...
		$userid 	= (isset($this->request->data["login"]["userid"])) ? $this->request->data["login"]["userid"] : "" ;
		$password 	= (isset($this->request->data["login"]["passwd"])) ? $this->request->data["login"]["passwd"] : "" ;
	
		if(!$this->BeAuth->login($userid, $password)) {
			$this->loginEvent('warn', $userid, "login not authorized");
			$this->userErrorMessage(__("Wrong username/password or no authorization"));
			$this->result=self::ERROR;
		}

		if(!$this->BeAuth->isValid) {
			$this->loginEvent('warn', $userid, "login blocked");
			$this->userErrorMessage(__("User login temporary blocked"));
			$this->result=self::ERROR;
		}
		...
		// redirect setup
		if(isset($this->request->data["login"]["URLOK"])) {
			$this->request->data['OK'] = $this->request->data["login"]["URLOK"];
		}
	}
	...
	public function recoverPassword($service_type="recover_password", $hash=null) {
		...
		if (!empty($service_type) || !empty($hash)) {
			...
			$this->render(null, null, APP . 'View' . DS."pages/change_password.tpl");
		}
	}
