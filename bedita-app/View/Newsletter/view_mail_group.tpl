...
<!--
var urlListSubscribers = "{$this->Html->url('/newsletter/listSubscribers')}";
...
$(document).ready(function() {
	...
	$("#assocCard").click( function() {
		submitSubscribers("{$this->Html->url('/newsletter/addCardToGroup/')}{$item.id|default:''}");		
	});
	
	$("#changestatusSelected").click( function() {
		submitSubscribers("{$this->Html->url('/newsletter/changeCardStatus/')}{$item.id|default:''}");
	});

	$("#deleteSelected").bind("click", function() {
		if(!confirm("{t}Do you want unsubscribe selected items?{/t}")) 
			return false ;	
		submitSubscribers("{$this->Html->url('/newsletter/unlinkCard/')}{$item.id|default:''}");
	});
});
...
<form method="post" id="updateForm" action="{$this->Html->url('saveMailGroups')}">	
