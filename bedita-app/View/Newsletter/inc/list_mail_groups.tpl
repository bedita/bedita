...
var urlDelete = "{$this->Html->url('deleteMailGroups/')}";
...
$(document).ready(function(){
	...
	<table class="indexlist">
		...
		{foreach from=$mailGroups item="grp" name="fc"}
			...
			<tr rel="{$this->Html->url('/newsletter/viewMailGroup/')}{$grp.id}">
				<td>
					{$grp.group_name}
				</td>
				<td>
		...
		{*
		<form id="form_{$grp.id}" method="post" action="{$this->Html->url('saveMailGroups')}">
