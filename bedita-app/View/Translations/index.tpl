...
var urlDelete = "{$this->Html->url('deleteTranslations/')}" ;
...
function changeStatusTranslations() {
	...
	if(status != "") {
		...
		oToDel = (oToDel=="") ? "" : oToDel.substring(1);
		$("#objects_selected").attr("value",oToDel);
		$("#formObject").attr("action", '{$this->Html->url('changeStatusTranslations/')}' + status) ;
		$("#formObject").get(0).submit() ;
		return false ;
	}
}
...
<div class="mainfull">
	...
	<form method="post" action="{$this->Html->url('/translations/index')}" id="formObject">
...
{/if}
	...
	{capture name="theader"}
		<tr>
			<th></th>
			<th>{$this->BeToolbar->order('title', 'master title')}</th>
			<th>{$this->BeToolbar->order('LangText.title', 'title')}</th>
			<th>{$this->BeToolbar->order('object_type_id', 'type')}</th>
			<th>{$this->BeToolbar->order('LangText.lang', 'language')}</th>
			<th>{$this->BeToolbar->order('LangText.status', 'Status')}</th>
		</tr>
	{/capture}
		...
		<tr class="obj {$translations[i].LangText.status}">
			...
			<td><a href="{$this->Html->url('view/')}{$oid}/{$olang}">{$translations[i].LangText.title|default:'<i>[no title]</i>'|truncate:38:true}</a></td>
			<td>
