...
<script type="text/javascript">
	var urlView = '{$this->Html->url("/multimedia/view/")}' ;
	...
	$(document).ready(function() { 
		...
		var optionsForm = { 
			...
			url: "{$this->Html->url('/multimedia/saveAjax')}"
		} ;
		...
		<ul class="menuleft insidecol catselector">
			<li><a href="javascript:void(0)" onClick="$('#mediatypes').slideToggle();">{t}Select by type{/t}</a></li>
				<ul id="mediatypes" {if empty($categorySearched)}style="display:none"{/if}>
					...
					<li class="ico_{$media_type} {if $cat==$media_type}on{/if}" rel="{$this->Html->url('/multimedia')}/index/category:{$media_type}">
						{$media_type}
					</li>
					{/foreach}
					<li class="ico_all" rel="{$this->Html->url('/multimedia')}">
						All
					</li>
