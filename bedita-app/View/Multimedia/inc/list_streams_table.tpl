<table class="indexlist" id="viewlist" style="display:block;">
	...
	{capture name="theader"}
		<tr>
			...
			<th>{$this->BeToolbar->order('name', 'FileName')}</th>
...
{strip}
		...
		<div style="width:{$thumbWidth}px; height:35px; overflow:hidden; border:4px solid white;">		
			...
			<a href="{$this->Html->url('view/')}{$objects[i].id}">
				...
				{$this->BeEmbedMedia->object($objects[i],$params,$htmlAttr)}
			</a>
						
			{elseif ($objects[i].provider|default:false)}
			
				{assign_associative var="htmlAttr" width="30" heigth="30"}
				<a href="{$filePath}" target="_blank">{$this->BeEmbedMedia->object($objects[i],null,$htmlAttr)}</a>
			
			{else}
			
				<a href="{$conf->mediaUrl}{$filePath}" target="_blank">
					<img src="{$this->Session->webroot}img/mime/{$objects[i].mime_type}.gif" />
				</a>
...
{/strip}
		...
		<td>{$objects[i].id}</td>
		<td><a href="{$this->Html->url('view/')}{$objects[i].id}">{$objects[i].title}</a></td>
