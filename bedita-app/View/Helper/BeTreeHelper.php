...
class BeTreeHelper extends AppHelper {
	...
	public function designBranchMobile($branch, $inputType = null, $parent_ids = array()) {
		...
		foreach ($branch as $section) {
			...
			if (empty($inputType)) {
				$url = $this->Html->url('/') . $this->request->params["controller"] . "/" . $this->request->params["action"] . "/id:" . $section["id"];
				if ( (!empty($this->request->params["named"]["id"]) && $this->request->params["named"]["id"] == $section["id"]) 
						|| !empty($this->request->params["pass"][0]) && $this->request->params["pass"][0] == $section["id"]) {
					$class = " class='on'";
				} else {
					$class = "";
				}
	...
	public function view($tree=array(), $inputType=null, $parent_ids=array()) {
		...
		if (!empty($tree)) {
			...
			foreach ($tree as $publication) {
				...
				if (empty($inputType)) {
					$url = $this->Html->url('/') . $this->request->params["controller"] . "/" . $this->request->params["action"] . "/id:" . $publication["id"];
					if ( (!empty($this->request->params["named"]["id"]) && $this->request->params["named"]["id"] == $publication["id"]) 
							|| !empty($this->request->params["pass"][0]) && $this->request->params["pass"][0] == $publication["id"]) {
						$class = " class='on'";
					} else {
						$class = "";
					}
