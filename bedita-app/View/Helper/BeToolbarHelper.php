...
class BeToolbarHelper extends AppHelper {
	...
	public function init(&$toolbar) {
		$this->request->params['toolbar'] = $toolbar;
	}
	...
	public function size() {
		return (isset($this->request->params['toolbar']['size'])?$this->request->params['toolbar']['size']:"" ) ;
	}
	...
	public function current() {
		return (isset($this->request->params['toolbar']['page'])?$this->request->params['toolbar']['page']:"" ) ;
	}
	...
	public function pages() {
		return (isset($this->request->params['toolbar']['pages'])?$this->request->params['toolbar']['pages']:"" ) ;
	}
	...
	public function changeDim($htmlAttributes = array(), $options = array(1, 5, 10, 20, 50, 100)) {
		if(!isset($this->request->params['toolbar']['dim'])) return "" ;
		...
		return $this->Form->select("", $options, $this->request->params['toolbar']['dim'], $htmlAttributes, false) ;
	}
	...
	public function changeDimSelect($selectId, $htmlAttributes = array(), $options = array(1 => 1, 5 => 5, 10 => 10, 20 => 20, 50 => 50, 100 => 100)) {
		...
		unset($data["dim"]);
		$url = Router::url($data) ;
		if($this->request->params["action"] == "index" && !preg_match("/\/(index$|index\/)/i", $url )) {
			$url .= "/".$this->request->params["action"];
		}
		$htmlAttributes['onchange'] = "document.location = '{$url}'+'/dim:'+ this[this.selectedIndex].value" ;

		return $this->Form->select($selectId, $options, $this->request->params['toolbar']['dim'], $htmlAttributes, false) ;
	}
	...
	public function changePage($htmlAttributes = array(),	$items = 5) {
		if(!isset($this->request->params['toolbar']['page'])) return "" ;
		...
		$pages = array() ;
		for($i = $this->request->params['toolbar']['page']; $i >= 1 ; $i--) {
			$pages[] =  $i ;
		}

		for($i = $this->request->params['toolbar']['page']; $i <= $this->request->params['toolbar']['pages'] ; $i++) {
			$pages[] =  $i ;
		}
		sort($pages) ;
		...
		return $this->Form->select("", $pages, $this->request->params['toolbar']['page'], $htmlAttributes, false) ;
	}
	...
	public function changePageSelect($selectId, $htmlAttributes = array(),	$items = 5) {
		...
		return $this->Form->select($selectId, $pages, $this->request->params['toolbar']['page'], $htmlAttributes, false) ;
	}
	...
	public function order($field, $title="", $image="", $htmlAttributes = array(), $dir=null) {
		...
		if(!isset($this->request->params['toolbar'])) return "" ;
		...
		if (!empty($image)) {
			$htmlAttributes["alt"] = __($htmlAttributes["alt"]);
	...
	private function _scroll($where, $title, $options, $disabledTitle, $disabledOption) {
		$page = (isset($this->request->params['toolbar'][$where]))? $this->request->params['toolbar'][$where] : false ;
		...
		$url = Router::url($data) ;
		return '<a title="go to '.$where.' page" href="' . $url . '">' . __($title) . '</a>';
	}
	...
	private function _output($text, $options) {
		return $this->output(
			sprintf(
					...
					$this->_parseAttributes($options, null, ' ', ''), __($text)
			)
		);
	}
	...
	public function getPassedArgs($otherParams=array()) {
		if (empty($otherParams))
			return array_merge($this->request->params["pass"], $this->request->params["named"]);
		else
			return array_merge($this->request->params["pass"], $this->request->params["named"], $otherParams);
	}
}
