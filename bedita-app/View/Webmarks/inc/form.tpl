...
var url="{$this->Html->url('/webmarks/checkUrl')}";
...
<form action="{$this->Html->url('/webmarks/save')}" method="post" name="updateForm" id="updateForm" class="cmxform">
				...
				{if !empty($relObjects.link)}
					{foreach from=$relObjects.link item="o"}
						<li>
							...
							<a title="{$o.created}"  href="{$this->Html->url('/')}{$o.ObjectType.module_name}/view/{$o.id}">{$o.title}</a>
						</li>
					{/foreach}
				{else}
