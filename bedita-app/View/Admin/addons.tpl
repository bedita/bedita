...
var updateUrl = "{$this->Html->url("/admin/updateAddon")}";
var diffUrl = "{$this->Html->url("/admin/showDiff")}";
{literal}
	...
	<table class="indexlist" style="float:left; width:49%; margin-right:10px;">
		...
		{if !empty($addons.models.objectTypes.on)}
			...
			{foreach from=$addons.models.objectTypes.on item="ot"}
				<tr>
					<td>
					<form action="{$this->Html->url('/admin/disableAddon')}" method="post">
					...
					{if $ot.update}
						...
						<input type="button" class="modalbutton" rel="{$this->Html->url('/admin/diffAddon/')}filename:{$ot.file}/type:models" value="{t}DIFF{/t}"/>
					{/if}
					</td>
				</tr>
			{/foreach}
		...
		{if !empty($addons.models.objectTypes.off)}
			...
			{foreach from=$addons.models.objectTypes.off item="ot"}
				<tr>
					...
					{else}
					<form action="{$this->Html->url('/admin/enableAddon')}" method="post">
						<td>
						<input type="hidden" name="path" value="{$ot.path}">
						<input type="hidden" name="enabledPath" value="{$ot.enabledPath}">
						<input type="hidden" name="name" value="{$ot.name}">
		...
		{if !empty($addons.models.others.on)}
			...
			{foreach from=$addons.models.others.on item="a"}
				<tr>
					...
					{if $a.update}
						<input type="button" class="js-update" value="{t}UPDATE{/t}"/>
						<input type="button" class="modalbutton" rel="{$this->Html->url('/admin/diffAddon/')}filename:{$a.file}/type:models" value="{t}DIFF{/t}"/>
					{/if}
					</td>
				</tr>
			{/foreach}
		...
		{if !empty($addons.behaviors.on)}
			...
			{foreach from=$addons.behaviors.on item="a"}
				<tr>
					...
					{if $a.update}
						<input type="button" class="js-update" value="{t}UPDATE{/t}"/>
						<input type="button" class="modalbutton" rel="{$this->Html->url('/admin/diffAddon/')}filename:{$a.file}/type:behaviors" value="{t}DIFF{/t}"/>
					{/if}
					</td>
				</tr>
			{/foreach}
		...
		{if !empty($addons.components.on)}
			...
			{foreach from=$addons.components.on item="a"}
				<tr>
					...
					{if $a.update}
						<input type="button" class="js-update" value="{t}UPDATE{/t}"/>
						<input type="button" class="modalbutton" rel="{$this->Html->url('/admin/diffAddon/')}filename:{$a.file}/type:components" value="{t}DIFF{/t}"/>
					{/if}
					</td>
				</tr>
			{/foreach}
		...
		{if !empty($addons.helpers.on)}
			...
			{foreach from=$addons.helpers.on item="a"}
				<tr>
					...
					{if $a.update}
						<input type="button" class="js-update" value="{t}UPDATE{/t}"/>
						<input type="button" class="modalbutton" rel="{$this->Html->url('/admin/diffAddon/')}filename:{$a.file}/type:helpers" value="{t}DIFF{/t}"/>
					{/if}
					</td>
				</tr>
			{/foreach}
