...
{if !empty($pluginModules.plugged)}
	...
	{foreach from=$pluginModules.plugged item="mod"}
		<li class="{$mod.name} {$mod.status}">
			<form action="{$this->Html->url('/admin/plugModule')}" method="post">
			{t}{$mod.label}{/t}
			...
			{if $mod.status == "on"}
				...
				<input type="button" rel="{$this->Html->url('/admin/toggleModule')}" value="{t}turn off{/t}"/>
			{elseif $mod.status == "off"}
				<input type="hidden" value="on" name="data[status]"/>
				<input type="button" rel="{$this->Html->url('/admin/toggleModule')}" value="{t}turn on{/t}"/>
			{/if}
			<input type="button" id="unplugButton" rel="{$this->Html->url('/admin/unplugModule')}" value="{t}plug-out{/t}"/>
			</form>
		</li>
	{/foreach}
	</ul>
