...
<form method="post" action="" id="formObject">
	...
	<tr>
		<th>{$this->Paginator->sort('Name', 'label')}</th>
		<th>{$this->Paginator->sort('Status', 'status')}</th>
		<th class="center">{$this->Paginator->sort('Occurrences', 'occurences')}</th>
		<th>Id</th>
		<th></th>
	</tr>
	...
	{section name="i" loop=$objects}
		<tr>
			<td>
				...
				&nbsp;&nbsp;
				<a href="{$this->Html->url('view/')}{$objects[i].id}">{$objects[i].label}</a>
				
			</td>
			<td>{$objects[i].status}</td>
			<td class="center">33</td>
			<td><a href="{$this->Html->url('view/')}{$objects[i].id}">{$objects[i].id}</a></td>
			<td><a href="{$this->Html->url('view/')}{$objects[i].id}">{t}details{/t}</a></td>
		</tr>
	{sectionelse}
