...
{foreach from=$objsRelated|default:'' item="c"}
	<tr class="obj {$c.status}">
		...
		<td>{if $c.num_of_editor_note|default:''}<img src="{$this->Html->webroot}img/iconNotes.gif" alt="notes" />{/if}</td>
		
		<td class="commands" style="white-space:nowrap">
			<input type="button" class="BEbutton golink" onClick="window.open($(this).attr('href'));" href="{$this->Html->url('/')}{$conf->objectTypes[$c.object_type_id].module_name}/view/{$c.id}" name="details" value="››" />
			{if !empty($c.fixed)}
