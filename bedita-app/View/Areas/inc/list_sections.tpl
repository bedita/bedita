		...
		{foreach from=$sections item="s"}
			...
			<tr class="obj {$s.status}">
				...
				<td class="checklist">
					...
					{if $s.menu == 0}
					<img title="{t}hidden from menu and canonical path{/t}" src="{$this->Html->webroot}img/iconHidden.png" style="height:30px; vertical-align:top;">
					{/if}
			...
			<td>
				...
				<td>{if $s.num_of_editor_note|default:''}<img src="{$this->Html->webroot}img/iconNotes.gif" alt="notes" />{/if}</td>
				...
				<td class="commands" style="white-space:nowrap">
					...
					href="{$this->Html->url('/')}areas/view/{$s.id}" name="details" value="››" />

					{if !empty($s.fixed)}
						<img title="{t}fixed object{/t}" src="{$this->Html->webroot}img/iconFixed.png" style="margin-left:10px; height:12px;" />
					{/if}
