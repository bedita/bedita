...
{assign var=branch value=$this->Html->params.named.branch|default:''}
...
$(document).ready(function(){
	...
	window.onbeforeunload = function () {
		...
		if (!$(".secondacolonna .modules label").hasClass("save") && formFieldToCheckData !== submitFormFieldSerialized) {
			formFieldToCheckSelector).serialize()) {
...
{/if}
	...
	$("div.insidecol input[name='clone']").click(function() {
		...
		if (cloneTitle) {
			...
			$("#updateForm").attr("action","{$this->Html->url('/')}{$submiturl}/cloneObject");
			$("#updateForm").submit();
		}
	});
	...
	$.datepicker.setDefaults({
	    ...
	    buttonImage: '{$this->Html->webroot}img/iconCalendar.gif', 
...
{*  {if !($this->Perms->isWritable($user.userid,$user.groups,$object.Permission))}
...
function autoSave() {
	...
	var submitUrl = "{$this->Html->url('/')}{$view->params.controller}/autosave/";
...
function updateEditors() {
	...
	var submitUrl = "{$this->Html->url('/pages/updateEditor/')}"+"{$object.id|default:''}";
