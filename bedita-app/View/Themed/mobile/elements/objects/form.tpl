	{strip}
	<form action="{$this->Html->url('/')}{$submiturl}/save" method="post" name="updateForm" id="updateForm" class="cmxform">
		...
		<ul data-role="listview">
			...
			<li data-role="fieldcontain">
				...
				<select id="data[destination][]" name="data[destination][]" multiple="multiple" data-native-menu="false">
					...
					{$this->BeTree->optionsMobile($tree, $params)}
				</select>
			</li>
			...
			<li class="ui-body ui-body-b">
				<fieldset class="ui-grid-a">
					<div class="ui-block-a"><button id="deleteButton" type="submit" data-theme="f" value="{if !empty($delparam)}{$this->Html->url($delparam)}{else}{$this->Html->url('delete/')}{/if}">{t}Delete{/t}</button></div>
					<div class="ui-block-b"><button id="saveButton" type="submit" data-theme="a" value="{$this->Html->url('/')}{$submiturl}/save">{t}Save{/t}</button></div>
				</fieldset>
			</li>
			...
			<li data-role="fieldcontain">
			    <fieldset data-role="controlgroup">
			    	<legend>Choose an action:</legend>
			         	<input type="radio" name="action" id="saveBEObject" value="{$this->Html->url('/')}{$submiturl}/save" checked="checked" />
			         	<label for="saveBEObject">{t}Save{/t}</label>

								<input type="radio" name="action" id="cloneBEObject" value="{$this->Html->url('/')}{$submiturl}/cloneObject" />
			         	<label for="cloneBEObject">{t}Clone{/t}</label>

								<input type="radio" name="action" id="delBEObject" value="{if !empty($delparam)}{$this->Html->url($delparam)}{else}{$this->Html->url('delete/')}{/if}"/>
