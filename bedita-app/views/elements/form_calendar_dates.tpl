...
<div class="daterow">
	...
	<input size=10 type="text" id="eventStart{$idx}" class="dateinput" name="data[DateItem][{$idx}][start_date]" value="{if !empty($d.start_date)}{$d.start_date|date_format:$conf->datePattern}{/if}"/>
	<input size=5 type="text"  id="timeStart{$idx}"  class="timeStart" name="data[DateItem][{$idx}][timeStart]" value="{if !empty($d.start_date)}{$d.start_date|date_format:'%H:%M'}{/if}" />
	
	<label>{t}end{/t}:</label>
	{strip}
	<input size=10 type="text" id="eventEnd{$idx}" class="dateinput" name="data[DateItem][{$idx}][end_date]" value="{if !empty($d.end_date)}{$d.end_date|date_format:$conf->datePattern}{/if}"/>
	{/strip}
	<input size=5 type="text"  id="timeEnd{$idx}"  class="timeEnd" name="data[DateItem][{$idx}][timeEnd]" value="{if !empty($d.end_date)}{$d.end_date|date_format:'%H:%M'}{/if}" />
...
<div class="newdaterow">
	...
	<label>{t}end{/t}:</label>
	{strip}
	<input size=10 type="text" id="eventEnd{$idx}" class="dateinput" name="data[DateItem][{$idx}][end_date]" value=""/>
	{/strip}
