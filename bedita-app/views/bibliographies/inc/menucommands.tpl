...
$(document).ready(function(){
	...
	$("div.insidecol input[@name='clone']").click(function() {
		$("#updateForm").attr("action","{/literal}{$html->url('/bibliographies/cloneObject')}{literal}");
...
<div class="secondacolonna {if !empty($fixed)}fixed{/if}">
	...
	<div class="modules">
	   <label class="{$moduleName}" rel="{$html->url('/bibliographies')}">{t}{$currentModule.label}{/t}</label>
	</div> 
