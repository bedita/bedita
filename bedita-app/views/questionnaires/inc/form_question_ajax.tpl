{foreach from=$objsRelated item="objRelated" name="o"}
	...
	<tr>
		...
		<td style="text-align:center">{$objRelated.question_difficulty|default:0}</td>
		<td style="text-align:center">{$objRelated.status}</td>
		<td style="white-space:nowrap">
			<input class="BEbutton golink" rel="{$html->url('/')}questionnaires/view/{$objRelated.id}" name="details" type="button" value="details">
