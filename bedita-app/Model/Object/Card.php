...
//	var $hasMany = array() ;
	...
	function afterSave($created) {
		...
		if (!empty($this->data["Card"]["joinGroup"])) {
			...
			if (empty($this->id))
				throw new BeditaException(__("Error saving card"), "Missing model id in afterSave.");
			...
			foreach ($this->data["Card"]["joinGroup"] as $joinData) {
				...
				if (!empty($joinData["mail_group_id"])) {
					...
					if (!$mailGroupCard->save($joinData))
						throw new BeditaException(__("Error creating join between card and groups"), "Saving error");
	...
	public function importVCardFile($cardFile, array $options = null) {
		...
		foreach ($cards as $c) {
			...
			if(!$this->save($data)) {
				throw new BeditaException(__("Error saving card"), print_r($c) . " \nvalidation: " . print_r($this->validationErrors, true));
			}
			$numSaved++;
		}
