			...
			foreach ($tags as $tag) {
				...
				if (!empty($tag))  {
					...
					if (empty($tagDB)) {
						...
						$this->create();
						if (!$this->save($tagDB)) {
							throw new BeditaException(__("Error saving tags"));
						}
					}
...
//		pr($tagsArray);exit;
	...
	public function checkMediaType($object_type_id, $mediatype) {
		...
		if(empty($category)) { // if media category doesn't exists, create it
			...
			$this->create();
			if(!$this->save($data)) {
				throw new BeditaException(__("Error saving category"), $this->validationErrors);
			}
			$category['id']=$this->id;
		}
		...
		foreach ($names as $n) {
			...
			if(empty($idCat)) {
				...
				if(!$this->save($dataCat)) {
					throw new BeditaException(__("Error saving category") . ": " . print_r($dataCat));
				}
				$idCat = $this->id;
			}
			$res[] = $idCat;
