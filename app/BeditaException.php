...
class BeditaException extends Exception
{
	...
	public function __construct($message = NULL, $details = NULL, $res  = self::ERROR, $code = 0) {
   		if(empty($message)) {
   			$message = __("Unexpected error, operation failed");
   		}
	...
	public function __construct($message = NULL, $details = NULL, $res  = self::ERROR, $code = 0) {
		...
		if (empty($message)) {
			if ($this->errorType == "unlogged")
				$messages = __("You have to be logged to access to this item");
			elseif ($this->errorType == "unauthorized")
				$messages = __("You aren't authorized to access to this item");
		}
