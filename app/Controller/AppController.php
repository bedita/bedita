...
class AppController extends Controller
{
	...
	final function beforeFilter() {
		...
		$this->initAttributes();
	 	...
	 	if(isset($this->request->data["login"]) || $this->name === 'Authentications') {
			return;
		}
	 ...
	 * If form contains:
	 * 		$this->request->data['OK'] or $this->request->data['ERROR']
	...
	final function beforeRender() {
		...
		if(isset($this->request->data[$this->result])) {
			$this->redirUrl($this->request->data[$this->result]);

		} elseif ($URL = $this->forward($this->request->action, $this->result)) {
			$this->redirUrl($URL);
	...
	private function redirUrl($url) {
		if(strpos($url, self::VIEW_FWD) === 0) {
			$this->request->action=substr($url, strlen(self::VIEW_FWD));
		} else {
			$this->redirect($url);
		}
	}
	...
	protected function updateHistory() {
		...
		if ($this->request->params["url"]["url"] != "/") {
			...
			if ( !empty($historyConf) && $this->historyItem !== null && !$this->RequestHandler->isAjax() && !$this->RequestHandler->isFlash()) {
				...
				$this->historyItem["url"] = ($this->request->params["url"]["url"]{0} != "/")? "/" . $this->request->params["url"]["url"] : $this->request->params["url"]["url"];
	...
	protected function checkLogin() {
		...
		if(!$this->BeAuth->isLogged()) {
			if ($this->RequestHandler->isAjax()) {
				throw new BeditaAjaxException(__("Session Expired"), array("output" => "reload"));
			}

			if ($this->view == 'Smarty') {
				echo $this->render(null, null, APP . 'View' . DS."home".DS."login.tpl") ;
			} elseif ($this->view == 'ThemeSmarty'){
				echo $this->render(null, null, APP . 'View' . DS."themed".DS.$this->theme.DS."home".DS."login.tpl") ;
			} elseif ($this->view == 'Theme'){
				echo $this->render(null, null, APP . 'View' . DS."themed".DS.$this->theme.DS."home".DS."login.ctp") ;
			} else {
				echo $this->render(null, null, APP . 'View' . DS."home".DS."login.ctp") ;
			}
		...
		if(isset($this->moduleName)) {
			...
			if ($moduleStatus != "on") {
				if ($this->RequestHandler->isAjax()) {
					throw new BeditaAjaxException(__("Module not available"));
				}
				$logMsg = "Module [". $this->moduleName.  "] status off";
				$this->handleError($logMsg, __("Module not available"), $logMsg);
			...
			if(!isset($this->modulePerms) || !($this->modulePerms & BEDITA_PERMS_READ)) {
				if ($this->RequestHandler->isAjax()) {
					throw new BeditaAjaxException(__("You haven't grants for this operation"));
				}
				$logMsg = "Module [". $this->moduleName.  "] access not authorized";
				$this->handleError($logMsg, __("Module access not authorized"), $logMsg);
				$this->redirect("/");
			}
	 ...
	 * If variable is null, use $this->request->params[url].
	 * Parameters:
	 * 0..N array:
	 * 		0	var name
	...
	protected function setup_args() {
		...
		for($i=0; $i < $size ; $i++) {
			...
			if(isset($this->request->params["url"][$args[$i][0]]) && !empty($this->request->params["url"][$args[$i][0]])) {
				$args[$i][2] = $this->request->params["url"][$args[$i][0]] ;
				$this->passedArgs[$args[$i][0]] = $this->request->params["url"][$args[$i][0]] ;

			} elseif(isset($this->request->params["named"][$args[$i][0]]) && !empty($this->request->params["named"][$args[$i][0]])) {
				$args[$i][2] = $this->request->params["named"][$args[$i][0]] ;
				$this->passedArgs[$args[$i][0]] = $this->request->params["named"][$args[$i][0]] ;
			...
			if(!is_null($args[$i][2])) {
				...
				$this->request->params["url"][$args[$i][0]] = $args[$i][2] ;
	...
	protected function loadModelByType($modelClass) {
		...
		if($model === false) {
			throw new BeditaException(__("Object type not found - ").$modelClass);
		}
		return $model;
	}
	...
	protected function checkObjectWritePermission($objectId) {
		...
		if(!$permission->isWritable($objectId, $this->BeAuth->user))
			throw new BeditaException(__("No write permissions on object"));
	}
	...
	protected function saveObject(BEAppModel $beModel) {
		...
		if(!$new) {
			$this->checkObjectWritePermission($this->request->data['id']);
		}
		...
		$tagList = array();
		if (!empty($this->request->params["form"]["tags"]))
			$tagList = $categoryModel->saveTagList($this->request->params["form"]["tags"]);
		$this->request->data["Category"] = (!empty($this->request->data["Category"]))? array_merge($this->request->data["Category"], $tagList) : $tagList;

		$fixed = false;
		if(!$new) {
			$fixed = ClassRegistry::init("BEObject")->isFixed($this->request->data['id']);
			if($fixed) { // unset pubblication date, TODO: throw exception if pub date is set!
				unset($this->request->data['start_date']);
				unset($this->request->data['end_date']);
			}
		}
		...
		if(!$beModel->save($this->request->data)) {
			throw new BeditaException(__("Error saving $name"), $beModel->validationErrors);
		}

		// handle tree. Section and Area handled in AreaController
		if(!$fixed && isset($this->request->data['destination']) && $beModel->name != "Section" &&  $beModel->name != "Area") {
			$this->BeTree->updateTree($beModel->id, $this->request->data['destination']);
		}
	}
	...
	protected function deleteObjects($model) {
		...
		if(!empty($this->request->params['form']['objects_selected'])) {
			$objectsToDel = $this->request->params['form']['objects_selected'];
		} else {
			if(empty($this->request->data['id']))
				throw new BeditaException(__("No data"));
			$objectsToDel = array($this->request->data['id']);
		}
		...
		foreach ($objectsToDel as $id) {
			...
			if ($beObject->isFixed($id)) {
				throw new BeditaException(__("Error, trying to delete fixed object!"));
			}

			if ($model != "Stream") {
				if(!ClassRegistry::init($model)->delete($id))
					throw new BeditaException(__("Error deleting object: ") . $id);
			} else {
	...
	protected function viewRevision(BEAppModel $beModel, $id, $rev) {
		if(empty($id) || empty($rev)) {
			throw new BeditaException(__("Missing object id or revision number"));
		}
		$versionModel = ClassRegistry::init("Version");
		$nRev = $versionModel->numRevisions($id);
		if($rev < 1 || $rev > $nRev) {
			throw new BeditaException(__("Wrong revision number"));
		}
...
abstract class ModulesController extends AppController {
	...
	protected function checkWriteModulePermission() {
		if(isset($this->moduleName) && !($this->modulePerms & BEDITA_PERMS_MODIFY)) {
				throw new BeditaException(__("No write permissions in module"));
		}
	}
	...
	protected function paginatedList($id, $filter, $order, $dir, $page, $dim) {
		...
		$this->request->params['toolbar'] = &$objects['toolbar'] ;
		// template data
	...
	public function changeStatusObjects($modelName=null) {
		...
		if(!empty($this->request->params['form']['objects_selected'])) {
			...
			foreach ($objectsToModify as $id) {
				...
				if ($beObject->isFixed($id)) {
					throw new BeditaException(__("Error: changing status to a fixed object!"));
				}
				$model->id = $id;
				if(!$model->saveField('status',$this->request->params['form']["newStatus"]))
					throw new BeditaException(__("Error saving status for item: ") . $id);
				$objectsListDesc .= $id . ",";
			}
	...
	public function assocCategory() {
		...
		if(!empty($this->request->params['form']['objects_selected'])) {
			...
			$this->Transaction->commit() ;
			$this->userInfoMessage(__("Added items association to category") . " - " . $category_id);
	...
	public function disassocCategory() {
		...
		if(!empty($this->request->params['form']['objects_selected'])) {
			...
			$this->Transaction->commit() ;
			$this->userInfoMessage(__("Removed items association to category") . " - " . $category_id);
			$this->eventInfo("removed items association to category " . $category_id);
		}
	}

	public function addItemsToAreaSection() {
		$this->itemsAreaSectionOp('add',$this->request->params['form']['objects_selected'],$this->request->data['destination']);
	}

	public function moveItemsToAreaSection() {
		$this->itemsAreaSectionOp('del',$this->request->params['form']['objects_selected'],$this->request->data['source'],false);
		$this->itemsAreaSectionOp('add',$this->request->params['form']['objects_selected'],$this->request->data['destination']);
	}

	public function removeItemsFromAreaSection() {
		$this->itemsAreaSectionOp('del',$this->request->params['form']['objects_selected'],$this->request->data['source']);
	}
	...
	private function itemsAreaSectionOp($op='add',$objects_to_assoc=array(),$area_section_id=null,$user_info=true) {
		...
		if(!empty($objects_to_assoc)) {
			...
			if(!($area_section = $modelLoaded->findById($area_section_id))) {
				throw new BeditaException(sprintf(__("Error loading section: %d"), $area_section_id));
			}
			$this->Transaction->begin() ;
			for($i=0; $i < count($objects_to_assoc) ; $i++) {
				if ($beObject->isFixed($objects_to_assoc[$i])) {
					throw new BeditaException(__("Error: modifying a fixed object!"));
				}
				...
				if ($op=='add' && !$is_already_present) {
					if(!$modelTree->appendChild($objects_to_assoc[$i],$area_section_id)) {
						throw new BeditaException( __("Error during append child"));
					}
				} else if ($op=='del' && $is_already_present) {
					if(!$modelTree->removeChild($objects_to_assoc[$i],$area_section_id)) {
						throw new BeditaException( __("Error during remove child"));
					}
				}
			}
			$this->Transaction->commit() ;
			...
			if($user_info) {
				$this->userInfoMessage(__("Items $op_str area/section") . " - " . $area_section['title']);
			}
	...
	protected function checkAutoSave() {
		...
		$beObject = ClassRegistry::init("BEObject");
		if(!$new) {
			$objectId = $this->request->data['id'];
			$status = $beObject->field("status", "id = $objectId");
			if($status == "on") {
				throw new BeditaException(__("Autosave: bad status"));
			}
			...
			if(!$permission->isWritable($objectId, $user)) {
				throw new BeditaException(__("Autosave: no write permission"));
			}
			...
			if(count($res) > 1) {
				throw new BeditaException(__("Autosave: other editors present"));
			}
		}

		if(!($this->modulePerms & BEDITA_PERMS_MODIFY)) {
			throw new BeditaException(__("Autosave: no module permission"));
		}
	}
	...
	protected function viewObject(BEAppModel $beModel, $id = null) {
		...
		if(isset($id)) {
			...
			if(!($obj = $beModel->findById($id))) {
				throw new BeditaException(__("Error loading $name: ").$id);
			}
			if(!$beModel->checkType($obj['object_type_id'])) {
               throw new BeditaException(__("Wrong content type: ").$id);
			}
	...
	protected function setSessionForObjectDetail($objects=null) {
		...
		if (!empty($objects) && strstr($this->request->here, $modulePath)) {
		...
		$baseModuleUrl = rtrim($this->request->base,"/") . "/" . $modulePath;
		// set backFromView session vars and reset prevNext if necessary
		if (!empty($this->request->here) && (strstr($this->request->here, $modulePath . "/index") || $this->request->here === $baseModuleUrl)) {
			$backURL = (empty($this->request->params["form"]["searchstring"]))? $this->request->here : rtrim($this->request->here,"/") . "/query:" . urlencode($this->request->params["form"]["searchstring"]);
	...
	public function view($id) {
		...
		if (!method_exists($this, $method)) {
			throw new BeditaException(__("Missing view method")." - ".$method);
		}
	...
	public function delete() {
		$modelName = ClassRegistry::init("BEObject")->getType($this->request->data["id"]);
		$method = "delete" . $modelName;
		if (!method_exists($this, $method)) {
			throw new BeditaException(__("Missing delete method")." - ".$method);
		}
		$this->request->action = $method;
		$this->{$method}();
	}
	...
	public function save() {
		if(!empty($this->request->data["id"])) {
			...
			$objTypeId = $this->request->data["object_type_id"];
			$modelName = Configure::read("objectTypes.$objTypeId.model");
		}
		$method = "save" . $modelName;
		if (!method_exists($this, $method)) {
			throw new BeditaException(__("Missing save method")." - ".$method);
		}
		$this->request->action = $method;
		$this->{$method}();
	}
