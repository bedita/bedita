 ...
 * $Id$
 */
class BeMailComponent extends Component {
	...
	public function sendMailById($msg_id, $to, $html=true) {
		if (empty($msg_id) || empty($to))
			throw new BeditaException(__("Missing message id or recipient"));

		$mailMsgModel = ClassRegistry::init("MailMessage");
		$mailMsgModel->containLevel("default");
		if (!$res = $mailMsgModel->findById($msg_id))
			throw new BeditaException(__("Error finding mail message " . $msg_id));
	...
	public function sendMail($data=array(), $createLog = true) {
		if (!$this->send($data)) {
			...
			throw new BeditaMailException(__("Mail delivery failed"), $this->Email->smtpError);
		}
		if($createLog) {
			...
			foreach ($msgToLock as $key => $message) {
				if (!empty($message["MailGroup"])) {
					...
					if (!$mailMsgModel->saveField("mail_status", "injob")) {
						throw new BeditaException(__("Mail message lock failed: id " . $message["id"]));
					}
					$msgIds[] = $message["id"];
				}
			}
					...
					foreach ($res as $groupCard) {
						...
						if ($jobModel->find("count", array(
							...
							if (!$jobModel->save($data)) {
								throw new BeditaException(__("Error creating jobs"));
							}
						}
					}
	...
	public function notify() {
		...
		foreach ($jobsToSend as $job) {
			...
			$jobModel->id = $job['MailJob']['id'];
			if (!$this->send($data)) {
				$this->log(__("Notification mail delivery failed") . "-" . $this->Email->smtpError);
	...
	private function prepareData(&$data) {
		...
		if (empty($data["to"])) {
			throw new BeditaException(__("Missing recipient"));
		}

		if (empty($data["from"])) {
			throw new BeditaException(__("Missing from field"));
		}

		if (empty($data["subject"])) {
			throw new BeditaException(__("Missing subject field"));
		}
