 ...
 * $Id$
 */
class BeFileHandlerComponent extends Component {
	...
	public function del($id) {
		...
		$mod = ClassRegistry::init($model);
	 	if(!$mod->delete($id)) {
			throw new BEditaDeleteStreamObjException(__("Error deleting stream object")) ;	
	 	}
	 	return true ;
	}
	...
	private function _createFromURL(&$data, $clone, $getInfoUrl) {
		...
		if(empty($data['uri']) || !$this->_regularURL($data['uri'])) {
			throw new BEditaURLException(__("URL not valid")) ;
		}
		if($getInfoUrl && $this->paranoid) {
			// Remote file management
			if(!ini_get('allow_url_fopen')) {
				throw new BEditaAllowURLException(__("You can't use remote file")) ;
			}
			// Get MIME type
		...
		if (!$clone) {
			...
			if (empty($data["id"])) {
				if ($this->isPresent($data['uri'])) {
					throw new BEditaFileExistException(__("Media already exists in the system")) ;
				}
			// modify
	...
	private function _createFromFile(&$data, $clone) {
		...
		if(empty($data['uri']) && empty($data['id'])) {
			throw new BeditaException(__("Missing temporary file in filesystem."));
		}
		if (!file_exists($data["uri"])) {
			throw new BEditaFileExistException(__("Resource " . $data["uri"] . " not valid"));
		}
		...
		if (!$clone && ($stream_id = $streamModel->field("id", array("hash_file" => $data["hash_file"]))) ) {
			throw new BEditaFileExistException(__("File already exists in the filesystem"),array("id"=>$stream_id)) ;
		}
	...
	private function _create(&$data) {
		if (!$modelType = $this->_getTypeFromMIME($data["mime_type"])) {
			throw new BEditaMIMEException(__("MIME type not found").": ".$data['mime_type']) ;
		}
		if (!empty($data["id"])) {
		...
		if(!($ret = $mediaModel->save($data))) {
			throw new BEditaSaveStreamObjException(__("Error saving stream object"), $mediaModel->validationErrors) ;
		}
	...
	public function getInfoURL(&$data) {
		...
		if (!($headers = @get_headers($data["uri"],1))) {
			throw new BEditaInfoException(__("URL unattainable"));
		}
