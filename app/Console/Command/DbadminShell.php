		...
		if (!empty($streams)) {
			foreach ($streams as $s) {
				...
				if (!preg_match($conf->validate_resource['URL'], $s["Stream"]["uri"])) {
					...
					if ($hash === false)
						 throw new BeditaException(__("Building Hash file failed"));
					$streamModel->id = $s["Stream"]["id"];
					if (!$streamModel->saveField("hash_file", $hash))
						throw new BeditaException(__("Error saving hash_file field"));
			...
			foreach ($videos as $v) {
				if ($v["provider"] == "youtube") {
					...
					if(!($this->BeBlip->getInfoVideo($v["video_uid"]) )) {
						throw new BEditaMediaProviderException(__("Multimedia  not found")) ;
					}
				...
				if (!empty($v["provider"])) {
					...
					if (!$videoModel->saveField("thumbnail", $thumbnail))
						throw new BeditaException(__("Error saving thumbnail field"));
