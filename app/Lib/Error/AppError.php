...
class AppError extends ErrorHandler {
	...
	public function handleException(array $messages) {
		...
		if(isset($current)) {
			try {
				...
				$this->controller->output = $viewObj->render(null, "error", APP . 'View' . DS."errors/error500.tpl");				
			}
		} else {
			header('HTTP/1.1 404 Not Found');
			$this->log($this->errorTrace);
			$this->controller->render(null, "error", APP . 'View' . DS."errors/error404.tpl");
			$this->sendMail($this->errorTrace);
		}
	...
	public function handleAjaxException(array $messages) {
		...
		App::import('View', "Smarty");
		$viewObj = new SmartyView($this->controller);
		echo $viewObj->render(null, "ajax", APP . 'View' . DS."errors/error_ajax.tpl");
	}
	...
	public function handleExceptionRuntime(array $messages) {
		...
		App::import('View', "Smarty");
		$viewObj = new SmartyView($this->controller);
		echo $viewObj->render(null, "error", APP . 'View' . DS."errors/error500.tpl");				
	}
	...
	public function handleExceptionFrontend(array $messages) {
		...
		App::import('View', "Smarty");
		$viewObj = new SmartyView($this->controller);
		echo $viewObj->render(null, "error", APP . 'View' . DS."errors/error404.tpl");				
	}
	...
	public function handleExceptionFrontAccess(array $messages) {
		...
		$viewFile = (file_exists(APP . 'View' . DS."pages".DS.$viewName.".tpl"))? APP . 'View' . DS."pages".DS.$viewName.".tpl" : APP . 'View' . DS."pages".DS.$viewName.".ctp";
		...
		if(!empty($mailSupport)) {
			...
			foreach ($dest as $d) {
				...
				if (!$jobModel->save($data)) {
					$this->log(__("Error creating mail job") . "-" . print_r($data),true);
				}
			}
	...
	function __outputMessage($template) {
		...
		App::import('View', "Smarty");
		$viewObj = new SmartyView($this->controller);
		echo $viewObj->render(null, "error", APP . 'View' . DS."errors/" . $tpl);				
	}
}
?>