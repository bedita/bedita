...
class DeleteObjectBehavior extends ModelBehavior {
	...
	function beforeDelete(&$model) {
		...
		if (!$tree->deleteAll(array("id" => $model->id))) {
			throw new BeditaException(__("Error deleting tree"));	
		}
		if (!$tree->deleteAll(array("object_path LIKE '%/".$model->id."/%'"))) {
			throw new BeditaException(__("Error deleting children tree"));
		}
			...
			foreach ($res["RelatedObject"] as $obj) {
				...
				if (in_array($obj["switch"],$relations)) {
					...
					if (!$model->delete($obj["object_id"]))
						throw new BeditaException(__("Error deleting related object "), "id: ". $obj["object_id"] . ", switch: " . $obj["switch"]);
				}
	...
	private function deleteAnnotations($object_id) {
		...
		if (!empty($aList)) {
			...
			foreach ($aList as $id) {
				...
				if (!$model->delete($id)) {
					throw new BeditaException(__("Error deleting annotation " . $modelClass), "id: ". $id);
				}
			}
		}
