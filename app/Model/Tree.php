...
class Tree extends BEAppModel
{
	...
	public function saveMenuVisibility($ids, $parent_id, $menu = null) {
		if (empty($ids) || empty($parent_id)) {
			throw new BeditaException(__("Missing mandatory data id and/or parent_id to save menu visibility"), array("ids" => $ids, "parent_id" => $parent_id));
		}
		if (is_numeric($ids)) {
			$ids = array($ids);
		}
		foreach ($ids as $id) {
			...
			if (!$this->id) {
				throw new BeditaException( __("Error saving visibility in menu and canonical paths"), "Error retrieving Tree model primary key " . $this->primaryKey . " for id=" . $id);
			}
			if (!$this->saveField('menu', $menu)) {
				throw new BeditaException( __("Error saving visibility in menu and canonical paths"), "Error saving Tree.menu field " . $menu . " for object " . $id);
			}
		}
	}
	...
	public function cloneStructure($id, array $options = array()) {
		...
		if ($objectTypeId == Configure::read("objectTypes.area.id")) {
			...
			if (!$Area->cloneObject($id, $options)) {
				throw new BeditaException(__("Error cloning Publication") . ": id =  " . $id, array("id" => $id));
			}
			...
			if (!empty($sections)) {
				...
				foreach ($sections as $s) {
					...
					$Section->create();
					if (!$Section->save($s)) {
						throw new BeditaException(__("Error cloning Section") . " " . $s["title"], array("id" => $sectionId));
					}
					...
					if (!$this->setPriority($newSectionId, $s["priority"], $s["parent_id"])) {
						throw new BeditaException(__("Error setting Section priority"), array("id" => $newSectionId, "priority" => $s["priority"]));
					}
		...
		if (!empty($children["items"])) {
			foreach ($children["items"] as $item) {
				if (!$this->appendChild($item["id"], $newBranchId)) {
					throw new BeditaException(__("Error cloning tree"), array("child id" => $item["id"]));

				}
				// set priority
				if (!$this->setPriority($item["id"], $item["priority"], $newBranchId)) {
					throw new BeditaException(__("Error setting contents priority"), array("id" => $item["id"], "parent_id" => $newBranchId, "priority" => $s["priority"]));
				}
			}
		}
	}
