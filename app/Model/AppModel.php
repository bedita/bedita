	...
	public function getDefaultDateFormat($value = null, $throwOnError = false) {
		...
		if(is_string($value) && !empty($value)) {
			...
			try {
				...
				if($throwOnError) {
					throw new BeditaException(__("Error parsing date. Wrong format"), array("date" => $value));
				} else {
...
//				$conditions[] 	= " prmsUserByID ('{$userid}', BEObject.id, ".BEDITA_PERMS_READ.") > 0 " ;
		...
		if ($tmp === false)
			throw new BeditaException(__("Error finding objects"));
		...
		if ($tmpCount === false)
			throw new BeditaException(__("Error counting objects"));
