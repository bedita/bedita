...
class Section extends BeditaCollectionModel
{
    ...
    public function promoteToArea($sectionId) {
    	...
    	if (!$Area->save($data)) {
    		throw new BeditaException(__("Error promoting section to publication"), array_merge(array("id" => $sectionId), $Area->validationErrors));
    	}
		...
		if (!$this->Tree->save($treeRow)) {
			throw new BeditaException(__("Error promoting section's row relative to trees table"));
		}
		...
		if (!empty($rowsToUpdate)) {
			...
			foreach ($rowsToUpdate as $row) {
				...
				if (!$this->Tree->save($row)) {
					throw new BeditaException(__("Error updating tree"), $row["Tree"]);
				}
			}
		}
	}
