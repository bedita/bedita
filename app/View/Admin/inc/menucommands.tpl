...
<div class="secondacolonna {if !empty($fixed)}fixed{/if}">
	...
	<div class="insidecol">
		...
		{if $view->action == "systemEvents"}
			...
			<form action="{$this->Html->url('/admin/deleteEventLog')}" method="post">
			<input type="submit" value="{t}delete all{/t}"/>
			</form>

		{elseif $view->action == "systemLogs"}
		
			<form action="{$this->Html->url('/admin/emptySystemLog')}" method="post">
			<input type="submit" value="{t}empty all{/t}"/>
			</form>

		{elseif $view->action == "emailInfo"}

			<form action="{$this->Html->url('/admin/deleteAllMailUnsent')}" method="post">
			<input type="submit" value="{t}delete all{/t}*"/>
		...
		{elseif $view->action == "emailLogs"}
			...
			<form action="{$this->Html->url('/admin/deleteAllMailLogs')}" method="post">
			<input type="submit" value="{t}delete all{/t}"/>
			</form>
