...
<form action="{$this->Html->url('/users/saveUser')}" method="post" name="userForm" id="userForm" class="cmxform">
			...
			<tr>
				...
				<td>
					<input type="{if !empty($userdetail.id)}password{else}text{/if}" name="pwd" value="{if empty($userdetail.id)}{$genpassword|default:''}{/if}" id="pwd"
				    	...
				    	title="{$this->Tr->t($conf->passwdRegexMsg)|default:''}"/>&nbsp;</td>
			</tr>
			<tr>
		...
		<tr>
			...
			<td>
				{if !empty($objectUser.card)}
					<a href="{$this->Html->url('/')}addressbook/view/{$objectUser.card.0.id}">YES</a>
				{else}
					NO
				{/if}	
			</td>
