<script language="javascript" type="text/javascript">
	var urlAddObjToAssBase = "{$this->Html->url('/newsletter/loadContentToNewsletter')}";
...
{if ($conf->mce|default:true)}
	{$this->Html->script("tiny_mce/tiny_mce")}
	...
	<script language="javascript" type="text/javascript">
		...
		initializeTinyMCE("{$cssUrl|default:$this->Html->url('/css/newsletter.css')}");
		...
		$(document).ready(function() {
			$("#changeTemplate").change(function() {
				...
				if (template_id != "") {
					...
					$("#msgDetails").load("{$this->Html->url('/newsletter/showTemplateDetailsAjax/')}" + template_id, function() {
				...
				if (cssBaseUrl === undefined) {
					cssPath = "{$this->Html->url('/css/newsletter.css')}";
				} else {
...
*}
	...
	<input class="modalbutton" type="button" value="{t}Get contents{/t}" rel="{$this->Html->url('/pages/showObjects/0/0/0/leafs')}" style="width:200px" />
