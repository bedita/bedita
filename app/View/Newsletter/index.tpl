...
<div class="mainhalf">
	<div class="tab"><h2>{t}Subscribers{/t}</h2></div>
		<ul class="bordered">
			...
			<li>
				<b><a href="{$this->Html->url('/newsletter/mailGroups')}">{t}View all{/t}</a></b> 
				&nbsp;&nbsp;|&nbsp;&nbsp;
				<b><a href="{$this->Html->url('/newsletter/mailGroups')}">{t}Import{/t}</a></b> 
			</li>
		</ul>
	...
	<div class="tab"><h2>{t}Newsletters {/t}</h2></div>
		...
		{if !empty($recentMsg)}
			...
			<tr>
				<td><a href="{$this->Html->url('/newsletter/viewMailMessage/')}{$msg.id}">{$msg.title}</a></td>
			...
			<tr>	
				<td colspan="2" style="border-bottom:0px;">
					<b><a href="{$this->Html->url('/newsletter/newsletters')}">{t}View all{/t}</a></b>
					&nbsp;&nbsp;|&nbsp;&nbsp;
					<b><a href="{$this->Html->url('/newsletter/viewMailMessage')}">{t}Create new{/t}</a></b>
				</td>
			</tr>
		{else}
...
<div class="mainhalf" style="margin-right:0px;">
	...
	<div class="tab"><h2>{t}Invoices{/t}</h2></div>
		<ul class="bordered">
			...
			<li>
				<b><a href="{$this->Html->url('/newsletter/invoices')}">{t}View invoices{/t}</a></b>
			</li>
		</ul>
	...
	<div class="tab"><h2>{t}Templates{/t}</h2></div>
		...
		{foreach from=$templates item="temp"}
			<li><a href="{$this->Html->url('/newsletter/viewMailTemplate/')}{$temp.BEObject.id}">{$temp.BEObject.title}</a></li>
		{foreachelse}
			<li>{t}No template available{/t}</li>
		{/foreach}
			<li>
				<b><a href="{$this->Html->url('/newsletter/templates')}">{t}View all{/t}</a></b>
					&nbsp;&nbsp;|&nbsp;&nbsp;
				<b><a href="{$this->Html->url('/newsletter/viewMailTemplate')}">{t}Create new{/t}</a></b> 
			</li>
		</ul>
