		...
		<td>
			<span id="user_name">
				{if !empty($object.User)}
					<a href="{$this->Html->url('/admin/viewUser/')}{$object.User.0.id}">{$object.User.0.userid|default:''}</a>
				{else}
					{t}no user data{/t}
				{/if}
			</span>
			...
			<input type="button" class="modalbutton" name="edit" value="  {t}promote as user{/t}  "
				rel="{$this->Html->url('/admin/showUsers')}"
		...
		<td>
			...
			{if isset($moduleList.comments) && $moduleList.comments.status == "on"}
				{if !empty($object.num_of_comment)}
					<a href="{$this->Html->url('/')}comments/index/comment_object_id:{$object.id}"><img style="vertical-align:middle" src="{$this->Html->webroot}img/iconComments.gif" alt="comments" /> ({$object.num_of_comment}) {t}view{/t}</a>
				{/if}
			{/if}
		</td>
	</tr>
