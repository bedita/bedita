...
urls['addToMailgroup'] = "{$this->Html->url('addToMailgroup/')}";
...
//-->
	...
	{capture name="theader"}
		<tr>
			<th></th>
			<th>{$this->BeToolbar->order('title','name')}&nbsp;&nbsp;&nbsp;&nbsp;{$this->BeToolbar->order('surname','surname')}</th>
{*			<th>{$this->BeToolbar->order('company_name','organization')}</th>*}
<th>{$this->BeToolbar->order('id','id')}</th>
			<th>{$this->BeToolbar->order('status','Status')}</th>
			<th>{$this->BeToolbar->order('modified','modified')}</th>
			<th>{t}is user{/t}</th>
			<th>{$this->BeToolbar->order('email','email')}</th>
			<th>{$this->BeToolbar->order('country','country')}</th>
			<th>{$this->BeToolbar->order('note','Notes')}</th>	
		</tr>
	{/capture}
