<div class="modules modulesmenu_d shadow">	
	...
	<nav>
		<ul>
			<li class="index"><a href="{$this->Html->url('/')}" title="BEdita home">home</a></li>
		{if !empty($moduleList)}
		...
		{foreach from=$publications item=item}
			...
			<li class="index"><a target="_blank" href="{$item.public_url}" title="{$item.public_name} | {$item.public_url}">
				<img class="smallicon" src="{$this->Html->webroot}img/iconUrl.png">{$item.public_url|truncate:32:'[…]':true:true}</a></li>
			{/if}
		{/foreach}
		{/if}
			<li class="index"><a href="{$this->Html->url('/logout')}" title="Exit">{t}exit{/t}</a></li>
		</ul> 
	</nav>
