...
$(document).ready(function(){ 
	...
	$("#searchMultimedia").bind("click", function() { 
		...
		loadMultimediaAssoc(
			"{$this->Html->url('/streams/showStreams')}/{$object_id|default:'0'}/" + textToSearch,
			true
		);
	} );
	...
	$("#searchMultimediaShowAll").click(function() { 
		loadMultimediaAssoc(
			"{$this->Html->url("/streams/showStreams")}/{$object_id|default:'0'}",
			false
		);
	} );
	...
	{if $toolbar|default:""}
		...
		$("#streamNextPage").click(function() { 
			urlReq = "{$this->Html->url("/streams/showStreams")}/{$object_id|default:'0'}/{$streamSearched|default:'0'}/{$toolbar.next}/{$toolbar.dim}";
			loadMultimediaAssoc(urlReq,	false);
		} );
		$("#streamPrevPage").click(function() { 
			urlReq = "{$this->Html->url("/streams/showStreams")}/{$object_id|default:'0'}/{$streamSearched|default:'0'}/{$toolbar.prev}/{$toolbar.dim}";
			loadMultimediaAssoc(urlReq,	false);
		} );
		$("#streamFirstPage").click(function() { 
			urlReq = "{$this->Html->url("/streams/showStreams")}/{$object_id|default:'0'}/{$streamSearched|default:'0'}/{$toolbar.first}/{$toolbar.dim}";
			loadMultimediaAssoc(urlReq,	false);
		} );
		$("#streamLastPage").click(function() { 
			urlReq = "{$this->Html->url("/streams/showStreams")}/{$object_id|default:'0'}/{$streamSearched|default:'0'}/{$toolbar.last}/{$toolbar.dim}";
			loadMultimediaAssoc(urlReq,	false);
		} );
		$("#streamPagDim").change(function() { 
			urlReq = "{$this->Html->url("/streams/showStreams")}/{$object_id|default:'0'}/{$streamSearched|default:'0'}/{$toolbar.first}/" + $(this).val();
			loadMultimediaAssoc(urlReq,	false);
		} );
	...
	<fieldset>
		...
		<table class="indexlist" id="streamPagList" style="clear: left;">
			...
			<tr class="rowList" id="tr_{$mobj.id}">
				...
				<a title="show details" href="{$this->Html->url('/multimedia/view/')}{$mobj.id}" target="_blank">
					{$this->BeEmbedMedia->object($mobj,$params, $attributes)}
				</a>
				</td>
