<!-- start upload block-->
{$this->Html->css('swfupload')}
{$this->Html->script("swfupload/SWFUpload-src")}
{$this->Html->script("swfupload/callbacks")}
...
var URLDelete		= '{$this->Html->url('/files/deleteFile')}' ;
...
function uploadProgress(file, bytesLoaded) {
	...
	progress.style.background = "#f0f0f0 url({$this->Session->webroot}img/swfupload/progressbar.png) no-repeat -" + (200 - percent) + "px 0";
}
...
window.onload = function() {
	swfu = new SWFUpload({
		upload_script : "{$this->Html->url('/files/upload')}",
		target : "SWFUploadTarget",
		flash_path : "{$this->Session->webroot}js/swfupload/SWFUpload.swf",
			...
			<div id="wrapper">
				<div id="content">
					...
					<div id="SWFUploadTarget" style="border:5px solid green;">
						<form id="uploadForm" 
						action="{$this->Html->url('/files/upload')}" method="post" enctype="multipart/form-data">
