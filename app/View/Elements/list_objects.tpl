...
urls['assocObjectsCategory'] = "{$this->Html->url('assocCategory/')}";
urls['disassocObjectsCategory'] = "{$this->Html->url('disassocCategory/')}";
...
<form method="post" action="" id="formObject">
	...
	{capture name="theader"}
		<thead>
		<tr>
			<th>{$this->BeToolbar->order('fixed', '&nbsp;')}</th>
			<th>{$this->BeToolbar->order('title', 'title')}</th>
			<th style="text-align:center">{$this->BeToolbar->order('id', 'id')}</th>
			<th style="text-align:center">{$this->BeToolbar->order('status', 'status')}</th>
			<th>{$this->BeToolbar->order('modified', 'modified')}</th>
			<th style="text-align:center">
				{assign_associative var="htmlAttributes" alt="comments" border="0"} 
				{$this->BeToolbar->order('num_of_comment', '', 'iconComments.gif', $htmlAttributes)}
			</th>			
			<th>{$this->BeToolbar->order('lang', 'lang')}</th>
			<th>{$this->BeToolbar->order('num_of_editor_note', 'notes')}</th>
		</tr>
		</thead>
	{/capture}
		...
		<tr class="obj {$objects[i].status}">
			...
			{if !empty($objects[i].start_date) && ($objects[i].start_date|date_format:"%Y%m%d") > ($smarty.now|date_format:"%Y%m%d")}
				...
				<img title="{t}object scheduled in the future{/t}" src="{$this->Html->webroot}img/iconFuture.png" style="height:28px; vertical-align:top;">
			
			{elseif !empty($objects[i].end_date) && ($objects[i].end_date|date_format:"%Y%m%d") < ($smarty.now|date_format:"%Y%m%d")}
			
				<img title="{t}object expired{/t}" src="{$this->Html->webroot}img/iconPast.png" style="height:28px; vertical-align:top;">
			...
			{elseif (!empty($objects[i].start_date) && (($objects[i].start_date|date_format:"%Y%m%d") == ($smarty.now|date_format:"%Y%m%d"))) or ( !empty($objects[i].end_date) && (($objects[i].end_date|date_format:"%Y%m%d") == ($smarty.now|date_format:"%Y%m%d")))}
				...
				<img title="{t}object scheduled today{/t}" src="{$this->Html->webroot}img/iconToday.png" style="height:28px; vertical-align:top;">

			{/if}
			
			{if !empty($objects[i].num_of_permission)}
				<img title="{t}permissions set{/t}" src="{$this->Html->webroot}img/iconLocked.png" style="height:28px; vertical-align:top;">
			{/if}
			...
			<td style="min-width:300px">
				<a href="{$this->Html->url('view/')}{$objects[i].id}">{$objects[i].title|truncate:64|default:"<i>[no title]</i>"}</a>
			...
			<td>{if $objects[i].num_of_editor_note|default:''}<img src="{$this->Html->webroot}img/iconNotes.gif" alt="notes" />{/if}</td>
		</tr>
