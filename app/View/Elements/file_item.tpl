...
{assign_concat var="linkUrl" 1=$this->Html->url('/multimedia/view/') 2=$item.id}
...
{strip}
	...
	{if strtolower($item.ObjectType.name|default:'') == "image"}
		...
		{if !empty($fileName)}
			...
			{$this->BeEmbedMedia->object($item,$params,$htmlAttr)}
			
			
		{else}
		
			<img  alt="{$item.mediatype|default:'notype'}" title="{$item.mediatype|default:'notype'} | {$item.title}" src="{$this->Html->webroot}img/iconset/88px/{$item.mediatype|default:'notype'}.png" />
	...
	{else}
		...
		{assign_associative var="params" presentation="thumb"}
		{$this->BeEmbedMedia->object($item,$params)}
