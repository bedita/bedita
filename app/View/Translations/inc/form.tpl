...
<form action="{$this->Html->url('/translations/save')}" method="post" name="updateForm" id="updateForm" class="cmxform">
...
{if !empty($object_master.relations.attach) && $object_master.object_type_id != $conf->objectTypes.image.id
	...
	<fieldset rel="multimedia">
		...
		<tr>
			<td>
				<a href="{$this->BeEmbedMedia->object($image, ['presentation' => 'full', 'URLonly' => true])}">
					{$this->BeEmbedMedia->object($image,
						[
							'presentation' => 'thumb',
							'mode' => 'crop',
							'width' => 100,
