...
<!--
var urlDelete = "{$this->Html->url('deleteSelected/')}";
...
var urlAddMultipleTags = "{$this->Html->url('addMultipleTags/')}";
var urlChangeStatus = "{$this->Html->url('changeStatus/')}";
...
<form method="post" action="" id="formObject">
	...
	<tr>
		...
		<th style="width:45px;">
			...
			<img class="tagToolbar viewcloud" src="{$this->Html->webroot}img/iconML-cloud.png" />
			<img class="tagToolbar viewlist" src="{$this->Html->webroot}img/iconML-list.png" />
			
		</th>
		
		<th><a href="{$this->Html->url('/tags/index/')}label/{if $order == "label"}{$dir}{else}1{/if}">{t}Name{/t}</a></th>
		<th><a href="{$this->Html->url('/tags/index/')}status/{if $order == "status"}{$dir}{else}1{/if}">{t}Status{/t}</a></th>
		<th><a href="{$this->Html->url('/tags/index/')}weight/{if $order == "weight"}{$dir}{else}1{/if}">{t}Occurrences{/t}</a></th>
		<th>Id</th>
		<th></th>
	</tr>
	<tbody id="taglist">
	{foreach from=$tags item=tag}
		<tr class="obj {$tag.status}">
			...
			<td>
				<a href="{$this->Html->url('view/')}{$tag.id}">{$tag.label}</a>
				
			</td>
			<td>{$tag.status}</td>
			<td class="center">{$tag.weight}</td>
			<td><a href="{$this->Html->url('view/')}{$tag.id}">{$tag.id}</a></td>
			<td><a href="{$this->Html->url('view/')}{$tag.id}">{t}details{/t}</a></td>
		</tr>
	{foreachelse}
		...
		<tr>
			<td colspan="10" class="tag graced" style="text-align:justify; line-height:1.5em; padding:20px;">
				...
				<span class="obj {$tag.status}">
					<a title="{$tag.weight}" class="{$tag.class|default:""}" href="{$this->Html->url('view/')}{$tag.id}">
						{$tag.label}
					</a>
				</span>
				{/foreach}
